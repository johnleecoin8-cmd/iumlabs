...
const GTMService = () => {
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      // Team flow cards — stagger reveal
      gsap.utils.toArray<HTMLElement>(".gtm-ed .tmf-card").forEach((card, i) => {
        gsap.from(card, { y: 50, opacity: 0, duration: .8, ease: "power3.out",
  ...
  return (
    <div className="gtm-ed" ref={containerRef}>
      ...
      {/* TEAM — normal scroll, same background */}
      <section className="team-flow">
        <div className="wrap">
          <div className="lbl">The Team</div>
          <h2 style={{ fontFamily: "var(--serif)", fontWeight: 300, fontSize: "clamp(2rem,4vw,3.2rem)", letterSpacing: "-.02em", marginBottom: "3rem" }}>
            Operators, not <strong>account managers.</strong>
          </h2>
          <div className="team-grid-flow">
            {[
              { name: "David", role: "CEO", img: teamHelen },
              { name: "Bennet", role: "COO", img: teamBennet },
              { name: "J", role: "CMO", img: teamJ },
              { name: "Kevin", role: "Head of BD", img: teamKevin },
              { name: "Lewis", role: "PR Manager", img: teamLewis },
              { name: "Rachel", role: "Designer", img: teamRachel },
              { name: "Suki", role: "Managing Partner", img: teamSuki },
              { name: "Hyukjae", role: "BD Manager", img: teamHyukjae },
              { name: "Helen", role: "Community Moderator", img: teamDavid },
            ].map(m => (
              <div key={m.name} className="tmf-card">
                <div className="tmf-photo"><img src={m.img} alt={m.name} loading="lazy" /></div>
                <div className="tmf-info"><h4>{m.name}</h4><span>{m.role}</span></div>
              </div>
            ))}
          </div>
        </div>
