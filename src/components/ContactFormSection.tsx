...
import { Mail, MapPin, Send, ArrowRight, Calendar, MessageCircle, Sparkles, ChevronRight, Check } from "lucide-react";
  ...
  return <section className="bg-[#0A0A0A]">
      <div className="border-t border-white/15">
        ...
        <div className="relative overflow-hidden">
          ...
          <div className="relative px-3 sm:px-4 md:px-6 pt-4 sm:pt-5 md:pt-6 pb-2 sm:pb-3 md:pb-4 py-[4px]">
            {/* Hero Title */}
          ...
          <div className="flex flex-col lg:flex-row">
            ...
            <div className="hidden lg:block lg:w-2/5 p-8 lg:p-12 border-r border-white/15">
              <div className="sticky top-24">
                ...
                {/* Email */}
                <a href={`mailto:${brand.email}`} className="group flex items-center justify-between mb-8 pb-8 border-b border-white/15 hover:border-white/25 transition-colors">
                  <div className="flex items-start gap-4">
                ...
                {/* Telegram */}
                <a href={brand.telegramLink} target="_blank" rel="noopener noreferrer" className="group flex items-center justify-between">
                  <div className="flex items-start gap-4">
                ...
                opacity: 0
                    ...
                    <form onSubmit={handleSubmit}>
                      ...
                      duration: 0.2
                            ...
                            ...formData,
                            name: e.target.value
                          })} required className="w-full bg-white/5 border border-white/10 rounded-lg px-3 py-2.5 text-sm text-white placeholder:text-white/40 focus:border-white/30 focus:bg-white/10 focus:outline-none transition-all min-h-[44px]" />
                              </div>
                              <div>
                            ...
                            ...formData,
                            budget: option
                          })} className={`px-3 py-2.5 text-[11px] sm:text-xs rounded-lg border transition-all min-h-[40px] font-medium active:scale-[0.97] ${formData.budget === option ? 'bg-white/20 border-white text-white' : 'bg-white/5 border-white/10 text-white/70 hover:border-white/30 hover:bg-white/10'}`}>
                                    {option}
                                  </button>)}
                              </div>
                            </div>
                            ...
                            {/* Next Button */}
                            <button type="button" onClick={() => setCurrentStep(2)} disabled={!step1Complete} className={`w-full flex items-center justify-center gap-2 py-3 text-sm font-semibold rounded-lg transition-all duration-300 min-h-[44px] active:scale-[0.98] ${step1Complete ? 'bg-white text-black hover:bg-white/90' : 'bg-white/10 text-white/40 cursor-not-allowed'}`}>
                              Continue
                      ...
                      duration: 0.2
                            ...
                            <div className="bg-white/5 border border-white/10 rounded-lg p-3 mb-3">
                              <div className="flex items-center justify-between mb-3">
                                ...
                                <button type="button" onClick={() => setCurrentStep(1)} className="text-xs text-white/50 hover:text-white transition-colors">
                                  Edit
                                </button>
                              </div>
                            ...
                            <div>
                              ...
                              <textarea placeholder="Tell us about your project, goals, and timeline..." value={formData.message} onChange={e => setFormData({
                          ...formData,
                          message: e.target.value
                        })} rows={5} className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm sm:text-base text-white placeholder:text-white/40 focus:border-white/30 focus:bg-white/10 focus:outline-none transition-all resize-none min-h-[140px]" />
                            </div>

                            {/* Action Buttons */}
                            <div className="flex flex-col sm:flex-row gap-3">
                              <button type="button" onClick={() => setCurrentStep(1)} className="sm:w-auto px-6 py-3.5 text-sm font-medium text-white/70 hover:text-white border border-white/20 rounded-lg transition-colors min-h-[48px]">
                                Back
                              </button>
                              <button type="submit" disabled={isSubmitting || !step2Complete} className={`flex-1 flex items-center justify-center gap-2 py-3.5 sm:py-4 text-sm sm:text-base font-semibold rounded-lg transition-all duration-300 min-h-[52px] active:scale-[0.98] ${step2Complete ? 'bg-white text-black hover:bg-white/90' : 'bg-white/10 text-white/40 cursor-not-allowed'}`}>
                                {isSubmitting ? <>
        ...
        <div className="lg:hidden border-t border-white/10 p-4 sm:p-6">
          <div className="flex flex-wrap gap-4 justify-center">
            <a href={`mailto:${brand.email}`} className="flex items-center gap-2 px-4 py-2.5 bg-white/5 border border-white/10 rounded-full text-sm text-white/70 hover:bg-white/10 transition-colors min-h-[44px]">
              <Mail className="w-4 h-4" />
              {brand.email}
            </a>
            <a href={brand.telegramLink} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2.5 bg-white/5 border border-white/10 rounded-full text-sm text-white/70 hover:bg-white/10 transition-colors min-h-[44px]">
              <Send className="w-4 h-4" />
              {brand.telegram}
            </a>
          </div>
