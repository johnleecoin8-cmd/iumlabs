...
const trustSignals = [{
  value: 10,
  suffix: '+',
  label: 'Years in Korea Crypto',
  labelKo: '한국 크립토 경력'
}, {
  value: 0,
  suffix: '',
  label: 'Incidents or Leaks',
  labelKo: '사고 및 정보유출'
}, {
  value: 100,
  suffix: '%',
  label: 'Client NDA Protected',
  labelKo: 'NDA 보호율'
}, {
  value: 5,
  suffix: '',
  label: 'Official Exchange Partners',
  labelKo: '공식 거래소 파트너'
}];
  ...
  return <motion.div initial={{
    ...
    delay: index * 0.1
      ...
      <div className="flex flex-wrap gap-2">
        {data.badges.map((badge, i) => <span key={i} className="px-3 py-1 text-xs font-medium rounded-full bg-muted/50 text-muted-foreground border border-border">
            {badge}
          </span>)}
      </div>
    </motion.div>;
...
const AdvantageItem = ({
  ...
  data: typeof insiderAdvantages[0];
  index: number;
}) => {
  const Icon = data.icon;
  return <motion.div initial={{
...
export const AuthoritySection = () => {
  ...
  return <section ref={sectionRef} className="relative py-24 bg-background overflow-hidden md:py-[20px]">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 md:px-12 lg:px-20">
        ...
        <div className="mb-20 md:mb-24">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {exchangeDNA.map((item, index) => <DNACard key={item.title} data={item} index={index} />)}
          </div>
        </div>
        ...
        <div className="mb-20 md:mb-24">
          ...
          <div className="grid md:grid-cols-2 gap-4 p-6 rounded-2xl border border-border bg-card/30">
            {insiderAdvantages.map((item, index) => <AdvantageItem key={item.title} data={item} index={index} />)}
          </div>
        </div>
        ...
        <div className="mb-20 md:mb-24" ref={trustRef}>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {trustSignals.map((item, index) => <TrustSignalCard key={item.label} data={item} index={index} isInView={isInView} />)}
          </div>
        </div>
