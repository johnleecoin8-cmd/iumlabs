        ...
        {services.map((service, index) => {
          ...
          return (
            <TiltCard
              ...
              max={10}
              scale={1.02}
              className={`animated-border-card ${service.gradientClass} group flex-shrink-0 w-[320px] md:w-[360px] snap-center relative p-6 md:p-8 bg-card/50 backdrop-blur-sm cursor-pointer overflow-hidden transition-all duration-500 ${
                visibleCards[index] 
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <div className="flex items-start justify-between mb-6">
                  <div className={`group/icon w-14 h-14 rounded-xl bg-gradient-to-br ${service.color} flex items-center justify-center transition-all duration-300 group-hover:scale-110`}>
                    <ServiceIcon className="w-7 h-7 text-foreground group-hover:animate-icon-bounce" />
                  </div>
        ...
        {services.map((_, index) => (
          <div
            ...
            className={`w-2 h-2 rounded-full transition-all duration-300 ${
              visibleCards[index] ? "bg-primary/50 scale-100" : "bg-border/50 scale-75"
            }`}
          />
        ))}
      </div>
