...
const ResearchHeroSection = () => {
  ...
  return <div className="relative h-full min-h-[calc(100vh-2rem)] flex flex-col justify-between overflow-hidden rounded-2xl sm:rounded-3xl">
      ...
      <div className="absolute inset-0 overflow-hidden">
        ...
        {!shouldDisableVideo && !hasVideoError && <video ref={videoRef} {...videoProps} className="absolute inset-0 w-full h-full object-cover z-10" style={{
        ...videoProps.style,
        filter: "brightness(0.35)"
      }}>
        ...
        <div className="absolute inset-0 bg-gradient-to-b from-[hsl(0,0%,4%,0.3)] via-transparent to-[hsl(0,0%,4%,0.95)] z-20" />
      </div>

      {/* Floating Service Tags - Desktop */}
      {serviceTags.map((tag, index) => <motion.div key={index} className={`absolute ${tag.position} hidden lg:block z-10`} initial={{
      opacity: 0,
      scale: 0.8
    }} animate={{
      opacity: 1,
      ...
      <div className="flex-1 flex items-center justify-center relative z-10 px-4 sm:px-6">
        <div className="max-w-7xl mx-auto text-center">
          {/* Main Headline */}
          <motion.h1 className="font-sans text-display-hero mb-4 sm:mb-6 mt-8 sm:mt-12" initial={{
          opacity: 0,
          y: 30
        }} animate={{
          opacity: 1,
          ...
          {/* Subtext */}
          <motion.p className="text-body-lg text-white/60 max-w-2xl mx-auto mb-8 font-light leading-relaxed" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          {/* CTA Button */}
          <motion.button onClick={() => document.getElementById('articles')?.scrollIntoView({
          behavior: 'smooth'
        })} className="group relative inline-flex items-center gap-3 px-8 py-4 bg-white text-black font-medium text-sm rounded-full overflow-hidden transition-all duration-300 hover:shadow-lg hover:shadow-white/20 hover:-translate-y-0.5" initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
      ...
      <div className="relative z-10 py-4 sm:py-6">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-6">
            {stats.map((stat, index) => <StatItem key={index} value={stat.value} label={stat.label} suffix={stat.suffix} isVisible={isVisible} delay={index * 100} />)}
          </div>
        </div>
      </div>
      ...
      <div className="relative z-10 border-t border-white/10 py-3 sm:py-4 overflow-hidden">
        ...
        <div className="flex items-center logo-marquee-slow ml-14 sm:ml-16">
          {[...categoryPills, ...categoryPills, ...categoryPills].map((pill, index) => <div key={index} className="flex items-center gap-1.5 sm:gap-2 mx-1.5 sm:mx-2 px-3 sm:px-4 py-1.5 sm:py-2 bg-zinc-900/80 rounded-full border border-white/10 hover:border-white/20 transition-all duration-300">
      ...
      <motion.div className="absolute bottom-20 sm:bottom-24 right-4 sm:right-8 z-10 flex items-center gap-2 sm:gap-3 group cursor-pointer" initial={{
      opacity: 0
    }} animate={{
      opacity: 1
    }} transition={{
      ...
      behavior: 'smooth'
        ...
        <div className="relative flex flex-col items-center">
          <div className="w-5 h-8 sm:w-6 sm:h-9 rounded-full border border-white/20 group-hover:border-white/40 transition-colors duration-300 flex justify-center pt-1.5">
            <motion.div className="w-1 h-1.5 sm:w-1.5 sm:h-2 rounded-full bg-white/60 group-hover:bg-primary transition-colors duration-300" animate={{
            y: [0, 8, 0]
          }} transition={{
            duration: 1.5,
            repeat: Infinity,
...
const StatItem = ({
  ...
  return <motion.div className="text-center group cursor-default" initial={{
    opacity: 0,
    y: 20
  }} animate={{
    opacity: 1,
