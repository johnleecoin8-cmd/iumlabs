  ...
  try {
    ...
    for (const project of parsedProjects) {
      try {
        ...
        // Calculate new score based on mindshare (scale to reasonable range)
        const newScore = project.mindshare * 100;
        
        // Update sparkline by appending new score (keep last 8 points)
        ...
        if (upsertError) {
          results.push({ ticker: project.ticker, status: "error", error: upsertError.message });
        } else {
          results.push({ ticker: project.ticker, status: "success" });
        }
      } catch (err) {
        results.push({ 
          ticker: project.ticker, 
    ...
    return new Response(
      JSON.stringify({
        success: true,
        message: `Synced ${parsedProjects.length} projects from 3ridge: ${successCount} success, ${errorCount} errors`,
        results,
        debug: {
          scrapedLength: markdown.length,
          parsedCount: parsedProjects.length,
          sampleProjects: parsedProjects.slice(0, 3),
        }
      }),
